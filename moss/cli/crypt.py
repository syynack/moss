...
def edit(key, crypt_file):
    ...
    MossCrypt().edit_encrypted_file(key, crypt_file)
...
@click.option('-k', '--key', default=None, help = 'Key to be used for encryption')
@click.option('-o', '--output-file', default=None, help = 'File for encrypted data to be written to')
@click.option('-p', '--plaintext-file', default=None, help = 'Target file to be encrypted')
def lock(json, key, output_file, plaintext_file):
    if not plaintext_file:
        print '-p/--plaintext-file is a required argument.'
        sys.exit(1)

    result = MossCrypt().encrypt_file(key, output_file, plaintext_file)
...
@click.option('-j', '--json', is_flag=True, help = 'Output in JSON format')
@click.option('-k', '--key', default=None, help = 'Key to be used for encryption')
@click.option('-o', '--output-file', default=None, help = 'File for decrypted data to be written to')
@click.option('-c', '--crypt-file', default=None, help = 'Target file to be decrypted')
def unlock(json, key, output_file, crypt_file):
    if not key:
        print '-k/--key is a required argument.'
        sys.exit(1)
    ...
    result = MossCrypt().decrypt_file(key, output_file, crypt_file)
